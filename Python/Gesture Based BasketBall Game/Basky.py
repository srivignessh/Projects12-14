...
class PygButton(object):
    def __init__(self, rect=None, caption='', bgcolor=WHITE, fgcolor=RED, font=None, normal=None, down=None, highlight=None):
...
class Developer(pygame.sprite.Sprite):
        def __init__(self, (xy), vector,msg='PSS'):
                pygame.sprite.Sprite.__init__(self)
                mytext =pygame.font.SysFont("None", 50).render(msg, True,self.newcolour()).convert_alpha()
                screen = pygame.display.get_surface()
                self.rect=mytext.get_rect()
                self.image=mytext
...
class Game(object):
    ...
    def Playing(self,screen,Bkg,BB,leap):
        #print 'playing'
        posy=0 ;posx=0
        self.ClearBackground(screen)
        screen.blit(Bkg,(0,0))
        while True:
            ...
            self.ClearBackground(screen)
            screen.blit(Bkg,(0,0))
            if not leap.frame.hands.is_empty:
                 posx=leap.frame.hands[0].palm_position[0]
                 posy=leap.frame.hands[0].palm_position[1]
                 screen.blit(BB,(320+posx,500-posy))
            pygame.display.update()
            #FPSCLOCK.tick(FPS)
            
    def Start_Game(self,screen,leap):

        #print "Game Begins"
        self.ClearBackground(screen)
        Start = PygButton((320, 400, 150, 50), 'Play')
        Bkg=pygame.image.load("court.jpg").convert()
        BB=pygame.image.load("bb.png")
        screen.blit(Bkg,(0,0))
        screen.blit(BB,(500,500))
        Start.draw(screen)
        pygame.display.update()
        while True:
            pressed=pygame.key.get_pressed()
            for event in pygame.event.get():
              ...
              if 'click' in Start.handleEvent(event):
                    self.Playing(screen,Bkg,BB,leap)
        
    def Rules(self,screen):
        #print "Rules"
        Text1=pygame.font.SysFont("comicsansms",30)
        self.Rule_h=Text1.render("Instructions",True,(0,0,100),None)
        Text2=pygame.font.SysFont("tohama",30)
        self.Rule_b1=Text2.render("Play the Game By Tribbling and Shooting with Hand",True,(0,100,100),None)
        self.Rule_b2=Text2.render("Powered With Leap Motion Python",True,(50,10,100),None)
        Back = PygButton((320, 400, 150, 50), 'Back')
        self.ClearBackground(screen)
        Bkg=pygame.image.load("simple.jpg").convert()
        screen.blit(Bkg,(0,0))
        Back.draw(screen)
        screen.blit(self.Rule_h,(310,80))
        screen.blit(self.Rule_b1,(150,200))
        screen.blit(self.Rule_b2,(240,250))
        pygame.display.update()
        while True:
            pressed=pygame.key.get_pressed()
            for event in pygame.event.get():
    ...
    def Quit_Game(self):
        #leap.controller.remove_listener(listener)
        pygame.quit()
        exit()
...
def main():
    ...
    #Sound
    sound=pygame.mixer.Sound('yikes.ogg')
    #Title
    Text=pygame.font.SysFont("comicsansms",30)
    Title=Text.render("Basket Ball",True,(100,0,0),None)
    #BackGround
    Bkg=pygame.image.load("simple.jpg").convert()
    #Buttons
    Start = PygButton((320, 200, 150, 50), 'Start Game')
    Instructions = PygButton((320, 300, 150, 50), 'Instructions')
    Quit = PygButton((320, 400, 150, 50), 'Quit')
    ...
    while True:
       ...
       for event in pygame.event.get():
          ...
          if 'click' in Start.handleEvent(event):
                sound.play()
                play.Start_Game(screen,leap)
          if 'click' in Instructions.handleEvent(event):
                sound.play()
                #play.clearBackground(screen)
                play.Rules(screen)
